...
class RecetteTeste (
    ...
    private lateinit var savon: RecetteSavon
    @BeforeEach
    fun setup() {
        dataInitializer.run()
        ...
        savon = RecetteSavon(
            ...
            surgraissageDesire = 5f,
            apportEnEau = 353.0833f,
            avecSoude = true,
            concentrationAgentAlcalin = 30f,
            qteAlcalin = 504.40475f,
            ligne = mutableListOf(
                Ligne(
                    LigneId(ingredientId = 1, recetteSavonId = 1),
                    quantiteIngredient = 500f,
                    pourcentage = 50f,
                    ingredient = huileOlive
                ),
                Ligne(
                    LigneId(ingredientId = 2, recetteSavonId = 1),
                    quantiteIngredient = 500f,
                    pourcentage = 50f,
    ...
    fun `test calculNonPondere`() {
        ...
        val ins = savon.resultat.find { it.caracteristique?.nom == "Indice INS" }?.score!!
    ...
    fun `test calculPondere`() {
        ...
        val volMousse = savon.resultat.find { it.caracteristique!!.nom == "Volume de mousse" }?.score!!
        val tenueMousse = savon.resultat.find { it.caracteristique!!.nom == "Tenue de mousse" }?.score!!
        val durete = savon.resultat.find { it.caracteristique!!.nom == "Dureté" }?.score!!
        val solubilite = savon.resultat.find { it.caracteristique!!.nom == "Solubilité" }?.score!!
        val sechage = savon.resultat.find { it.caracteristique!!.nom == "Séchage" }?.score!!
